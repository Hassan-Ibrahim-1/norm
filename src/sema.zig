...
const Sema = struct {
    ...
    fn analyzeGlobalSymbols(s: *Sema, stmts: []Ast.Stmt, nir_stmts: *std.ArrayList(Nir.Stmt)) void {
        ...
        for (ordered_decls) |decl| {
            const nir_type = if (decl.type_expr) |t| s.analyzeTypeExpr(t) else s.inferType(decl.value.?);
            const sym = s.sym_table.find(decl.ident.lexeme, s.sym_table.top_scope);
            sym.type = nir_type;
        }
    ...
    fn resolveGlobals(s: *Sema, decl_list: []Ast.Stmt.VarDecl) []Ast.Stmt.VarDecl {
        const Decl = struct {
        ...
        for (decl_list) |decl| {
            ...
            decl_map.putAssumeCapacity(
                decl.ident.lexeme,
                .{ .status = .unresolved, .dependencies = ident_list.items, .stmt = decl },
            );
        }

        var ordered_decls: std.ArrayList(Ast.Stmt.VarDecl) = .empty;
